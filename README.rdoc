...
Merbish is a Rails 3 plugin that extends your controller action methods to simply look like a general Ruby method with meaningful parameters.
...
Merbish provieds a custom scaffold controller generator that overwrites the default scaffold generator.
...
* It's quite easy to comprehend what's the input value for each action
...
Each action method parameter name corresponds to +params+ key. For example, the following beautifully written nested show action works perfectly (this might not be a very good example of effective querying, but that's another story).
